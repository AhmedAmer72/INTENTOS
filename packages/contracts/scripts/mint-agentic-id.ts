...
function agentIdNumber(): number {
  ...
  if (!raw) {
    if (hre.network.name === "galileo") return 361;
    throw new Error("AGENT_ID is required in Agentic ID metadata (mainnet id from register-agent).");
  }
...
async function main() {
  const address = process.env.AGENTIC_ID_ADDRESS;
  if (!address) throw new Error("AGENTIC_ID_ADDRESS is required — deploy wave45 first");
  ...
  const nft = await hre.ethers.getContractAt("IntentosAgenticId", address);
  const tx = await nft.mint(deployer.address, encryptedURI, metadataHash);
  const receipt = await tx.wait();
  const tokenId = await nft.ownerOf(1n).then(() => 1n).catch(() => 1n);
  let minted = tokenId;
  ...
  const oracle = process.env.VERIFIER_ORACLE_PRIVATE_KEY
    ? new hre.ethers.Wallet(process.env.VERIFIER_ORACLE_PRIVATE_KEY).address
    : deployer.address;
  await (await nft.authorizeUsage(minted, oracle, "0x01")).wait();
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        ...prev,
        IntentosAgenticId: address,
        agenticTokenId: minted.toString(),
        agenticEncryptedURI: encryptedURI,
        agenticMetadataHash: metadataHash,
        agenticMintTx: receipt?.hash,
      },
      null,
      2,
    ) + "\n",
  ...
  console.log("Set AGENTIC_ID_TOKEN=" + minted.toString());
